...
public class Prob2346 {
    ...
    public void solve() throws IOException{
        ...
        String balloonsLine = br.readLine();
        String[] balloonsSplit = balloonsLine.split(" ");

//        List<Integer> balloons = new ArrayList<>(); // ballons를 ArrayList로 하면 메모리초과 발생
        ...
        while (true) {
            int pos = balloons.pollFirst();
            ans.append(pos + " ");
            if (balloons.isEmpty()) {
                break;
            }

            int shift = Integer.valueOf(balloonsSplit[pos - 1]);
            shift = shift > 0 ? shift - 1 : shift;

            if (shift > 0) {
                while (0 < shift) {
                    int removed = balloons.pollFirst();
                    balloons.add(removed);
                    shift--;
                }
            } else {
                while (shift < 0) {
                    int removed = balloons.pollLast();
                    balloons.addFirst(removed);
                    shift++;
                }
            }
